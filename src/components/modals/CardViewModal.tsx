...
    // 💡 修正1: 論理削除 (onRemove) / 復元 (onRestore) / 物理削除 (onPhysicalDelete) を一本化し、物理削除 (onDelete) のみに変更
    onDelete: (cardId: string) => Promise<void>; 
    packRaritySettings: RarityConfig[];
    currentPackName: string;
    currentPackId: string;
    // 💡 修正2: isAllViewMode を削除 (廃止)
}
...
    // 💡 修正5: 物理削除ロジック (旧 onPhysicalDelete の責務を 'onDelete' として一本化)
...
        try {
            await onDelete(localCard.cardId); // onDelete に委譲
            handleClose();
        } catch (error) {
            alert('カードの削除に失敗しました。');
            console.error(error);
...
    // 💡 修正6: onRemove, onRestore, onPhysicalDelete, showAdvancedControls のロジックは全て削除
...
    
    // 💡 修正7: showAdvancedControls の計算ロジックを削除（不要）

    return (
        <Dialog open={open} onClose={handleClose} maxWidth="lg" fullWidth>
            <DialogTitle>
                {isNew ? '新規カードの作成' : `カード「${card?.name}」の編集`}
                {/* 💡 修正8: 論理削除状態の表示を削除 (isInStore 廃止) */}
            </DialogTitle>
...
            </DialogContent>
            <DialogActions>
                {/* 💡 修正9: 特殊ボタンの表示制御・ボタン自体を削除 */}
                
                {/* 💡 修正10: 削除ボタン (isNew でない場合に表示) */}
                {!isNew && (

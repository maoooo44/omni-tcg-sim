...
# TODO
- プリセット（カード，パック，デッキ）をまだ実装できていない．まずは型定義から
- GCのユーザー設定
- アーカイブ関連
- CSVインポート時の挙動（レアリティが存在しないときデフォルト値で上書きされていない．自動採番についても考える必要がある）
- 新規デッキ作成時，無効なＵＲＬになる
- ZIPエクスポートでエクスポート対象のデッキがないときエラーがでる（デッキがなくても出力すべし）
- パック開封画面で裏面画像が反転している
- 開封アニメの挙動がおかしい
- 開封の待ち時間は3秒で十分
- カード裏面画像が設定できない
- パック編集画面に「パック編集: 新規パック」という文章
- パックを削除するボタンが「ストアから除外」に．
- 閲覧モード時にカードモーダルを開けない
- カードを削除して保存してもそれが反映されていない．これはおそらく，カードを削除した場合，その削除されたカードを検知してDBから削除する必要があるが，おそらくセーブしかされていないため
- カードプールのレアリティのフィルタリングがダミーのまま
- コレクションをリセットボタンは不要
- カードモーダルは，閲覧時と編集時で埋め込みを変えるのがよいだろうか．
- 閲覧時にはカードプールにおける枚数や売買を行えるようにしたい
- カードモーダルのUIが微妙
- カードモーダルも段々大きくなるのやめて欲しい．カスタムフィールドとかはその部分だけでスクロールできるようにしたい
- カードモーダルのカスタムフィールドを追加したとき．ゴミ箱マークが出ない．あと追加ボタンが意味をなしていない．あとブール値がチェックボックスなのがなんだか微妙．falseなら削除すればよいしタグ扱いでよくね．あとやっぱり型ごとにカスタムフィールドを表示するのがよさそう
- カスタムフィールドが最初から10個有効になっているのが駄目
- モーダルはかなり大きく作って画面内スクロールでも画面外スクロールでもどっちでもよいか？いや画面内スクロールの方が良い気がする
- カードを削除するとき二重で警告が出る．そもそも警告を出すかはユーザー設定で決められた方がよいかも
- 再採番機能なども作ってもよいだろう．そもそもカードはインポートによる導入が多いだろうし
- パックやカードの画像表示の大きさが適切ではない，あと画面に応じた表示にもしたい
- デッキ編集画面のUIがかなり微妙だったように思う
- パック編集画面のUIも洗練させたい．特にパック入力フォームに表示されていないメタデータが多い
- それにパックにもカスタムフィールドを設定できるようにする必要がある
- レアリティ編集のモーダルが入力フォームにじゃっかん被さっている．それに段々モーダルが大きくなっていくのが深いなのではじめから大きさを一定にしたい
- 確率は現在1になるようにしているが，100%単位の方が分かりやすいような．あと今0.01%単位なのはやりすぎ
- デッキは5枚や10枚並べがよさそう．ていうかパックもだけど，並べ方を2,3,5,10,20枚くらいで変更できるようにしたい
- デッキは並べた画像を画像出力できるようにしたいが，そんなのできるのか（画像はurlから表示しているので）
- 現在選択しているカードはカード名表示を上に出すようにしているけど，画像を大きくするのがいいかも
- スマホの個別カード表示はポケポケが優れていると思う．てかウェブは個別ウィンドウ表示多くて．あれはダメでしょ．横長の全画面表示の場合は，サイファやバトスピみたいに左側に画像で右側にテキストが良いと思う
- ゴッドモードからフリーモードに直接遷移できてしまうようになっている．またDTCGモードからフリーモードになるときにカードプールを削除するのではなく，フリーモードからDTCGモードに遷移したときにカードプールを削除するように直す
- カードプールに追加されたはずのカードが永続化されていない（再読み込みすると削除される）
- カードプールの個数表示がグリッド表示に適応できていない
- おそらくインポート時にカスタムフィールド名を使用できるようにしたせいで，インポートが上手く行かない．正直エラーの元の気がするので，再度インポート時はカスタムフィールドを使用してもらうようにするべき．しかしCSVファイルにフィールド名を含めたいという需要があると思うので，コメントアウトができるようにするとよいかも
- パックを新規作成したとき，パックの情報を変更するだけではダーティーチェックがオンにならない

- カスタムフィールドをユーザーデータから廃止し，プリセットで定義するようにした．ソート等のときはプリセットを参照し，custom_Xxx_Yyyをフィールド値に置き換えればよい
